...
class AppointmentController {
  ...
  async store(req, res) {
    try {
      ...
      if (!isCollaborator) return res.status(400).json({
        message: 'Colaborador não localizado.'
      });

      if (req.userId === Number(collaborator_id)) return res.status(400).json({
      ...
      if (CollaboratorNotAvailable) return res.status(400).json({
      ...
      const formattedHour = new Date(date).toLocaleString('pt-BR', { hour: '2-digit' });
      const formattedSeconds = new Date(date).toLocaleString('pt-BR', { minute: '2-digit' }); // Verify why 2-digit is not being applied.
      let contentDateText;
      ...
      if (user_name) await Notification.create({
